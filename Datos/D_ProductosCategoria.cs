        ...
        public List<ProductosCategoria> ProductosInicioDescuento()
        {
            ...
            using (SqlConnection oconexion = new SqlConnection(Conexion.conexion))
            {
                try
                {
                    ...
                    query.AppendLine("inner join tallasropa tr on tr.idtallaropa = p.idtallaropa");
                    query.AppendLine("inner join marca m on m.idmarca = p.idmarca");
                    query.AppendLine("WHERE p.descuento > 0");
        ...
        public List<ProductosCategoria> ProductosInicioNuevos()
        {
            ...
            using (SqlConnection oconexion = new SqlConnection(Conexion.conexion))
            {
                try
                {
                    ...
                    query.AppendLine("inner join tallasropa tr on tr.idtallaropa = p.idtallaropa");
                    query.AppendLine("inner join marca m on m.idmarca = p.idmarca");
                    query.AppendLine("WHERE p.fecharegistro >= CONVERT(DATE, GETDATE()) AND p.fecharegistro < DATEADD(MONTH, 2, CONVERT(DATE, GETDATE()))");
